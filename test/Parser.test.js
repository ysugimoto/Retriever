...
describe('Parser class testcase', function() {
    //var tpl = '{{foo}}hogehoge{{bar}}{{loop section}}{{data}}{{/loop}}{{if baz}}{{baz}}{{/if}}';
    var p;
    var tpl = fs.readFileSync('./fixture/testTemplate.html', {encoding: 'utf8'});
    ...
    it.skip('parse loop when parameter exists', function() {
        expect(p.parse({section: [{data:'a'},{data:'b'}]})).to.be.a('string').and.eql('ab');
    });

    it.skip('parse loop when parameter is an array', function() {
        expect(Parser.make('{{loop list}}{{@data}}{{/loop}}').parse({list: [1,2,3]})).to.be.a('string')
            .and.eql('123');
    });

    it.skip('access to parent parameter object in loop process', function() {
    ...
    it('parse condition test', function() {
        ...
        expect(Parser.prototype._parseCondition(cond, ['obj'])).to.be.a('string')
